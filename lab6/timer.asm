...
;Пример: TIMER.COM 13 37 0 0 0 10
.model tiny  
.386
.code
    org 100h               ; Размер PSP (program segment prefix)
...
stopHour        db 0       ;
stopMinutes     db 0       ; Время прекращения вывода сигнала
stopSeconds     db 0       ;
                           
badCMDArgsMessage db "Bad command-line arguments. Usage: start time [h m s] and duration time [h m s]", '$'
 
isAlarmOn db 0
                           
widthOfBanner   equ 40     ; Ширина выводимого баннера
allWidth        equ 80     ; Полная ширина консоли DOS-box
blue            equ 1420h  ; Цвета + выводимые символы. 4020h : 14 - код синего цвета, 20 - код символа залитого прямоугольника
white           equ 7020h  
black           equ 0020h

wakeUpText  dw widthOfBanner dup(blue)
            dw 4 dup(blue), white, 5 dup(blue), white, 2 dup(blue), 2 dup(white), blue, white, blue, 2 dup(white), blue, 3 dup(white), 4 dup(blue), white, 2 dup(blue), white, blue, 3 dup(white), 4 dup(blue)
            dw 4 dup(blue), white, 5 dup(blue), 5 dup(white, blue), blue, white, 6 dup(blue), white, 2 dup(blue), 3 dup(white, blue), 3 dup(blue)
            dw 5 dup(blue), 3 dup(white, blue), blue, 3 dup(white), blue, 2 dup(white), 3 dup(blue), 2 dup(white), 5 dup(blue), white, 2 dup(blue), white, blue, 3 dup(white), 4 dup(blue)
            dw 5 dup(blue), 3 dup(white, blue), 4 dup(blue, white), 2 dup(blue), white, 6 dup(blue), white, 2 dup(blue), 2 dup(white, blue), 5 dup(blue)
            dw 6 dup(blue), 2 dup(white, blue), 2 dup(blue), 3 dup(white, blue), 2 dup(white), blue, 3 dup(white), 5 dup(blue), 2 dup(white), 2 dup(blue), white, 6 dup(blue)
            dw widthOfBanner dup(blue)
...
handler PROC                          ; Новый обработчик прерывания
    pushf                             ; сохраняем флаги на стеке (имитация 6 байт на стеке, для корректного iret)
                                      ; iret как и retf, retn, только iret (ну вы поняли)
    call cs:intOldHandler             ; Вызываем старый обработчик прерывания
    push ds                           ; push ds - сюда передастся управление после вызова старого обработчика
    push es
    push ax
...
stopCheck:                            ; Проверка на возможность выключения будильника
    ...
    mov dl, isAlarmOn                     ; Определяем текущее состояние будильника
...
setHandler PROC                       ; Установка нового обработчика прерываний. Результат ax=0 если нет ошибок, иначе ax!=0 
    ...
    mov word ptr intOldHandler, bx    ; смещение
    mov word ptr intOldHandler + 2, es; сегмент
